...
router.post('/test_connection', function(req, res) {
    var connection = req.body;
    mongodb.connect('mongodb://' + connection.server + ':' + connection.port + '/admin',{
        db: {
...
router.post('/test_repl_connection', function(req, res) {
    var connection = req.body;
    var host = connection.server + ':' + connection.port;
    for (var i = 0; i < connection.replicaSet.childs.length; i++) {
        host += ',' + connection.replicaSet.childs[i].host + ':' + connection.replicaSet.childs[i].port;
    }
    var url = 'mongodb://' + host + '/' + connection.replicaSet.db + '?replicaSet=' + connection.replicaSet.replicaSetName;
